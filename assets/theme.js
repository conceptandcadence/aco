      ...
      for (const campaign in this.campaigns) {
        if (this.campaigns[campaign]?.tag && this.campaigns[campaign]['status'] == "active") {
          ...
          for (const item in Rebuy.Cart.cart.items) {
            if (Rebuy.Cart.cart.items[item].product.tags && Rebuy.Cart.cart.items[item].product.tags.includes(campaignTag)) {
  ...
  getProductCountNeeded(campaign) {
    ...
    if (Rebuy?.Cart?.cart?.items.length > 0) {
      if (campaign?.tag && campaign.status == "active") {
        for (const item in Rebuy.Cart.cart.items) {
          if (Rebuy.Cart.cart.items[item].product.tags && Rebuy.Cart.cart.items[item].product.tags.includes(campaign.tag)) {
...
var header = $("body #shopify-section-header header");
var html = $("html");
var body = $("body");
